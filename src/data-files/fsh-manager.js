...
class DataFileRecorder {
	...
	async removeRecord(fileId) {
		for (let record of this.fileDataItem) {
			if (
				record.filename === fileId.filename &&
				record.version === fileId.version
			) {
...
export class FshManager extends HLMApplication {
	 ...
	 */
	async onFileLoaded(ev, fileName, options) {
		const fileRecord = await constructFileRecord(ev, fileName);
		await this.checkFileRecordExists(fileRecord, ev, options?.targetFile);
	}
	...
	async confirmOverwriteCallback(proceed, args) {
		if (proceed) {
			...
			for (let record of args.fshManager.dataFiles) {
				if (
					record.filename === args.fileId.filename &&
					record.version === args.fileId.version
				) {
			...
			await args.fshManager.readFile(
				args.fileId,
				args.newFile,
				args.oldFile
			);
...
async function readDataJson(fileData, fileName, fileId, oldFile, dialog) {
	...
	if (dataTypes != null) {
		...
		ui.notifications.info(
			`Can't identify item types for ${fileName}, skipping...`
		);
	}
}
...
async function createItem(itemName, jsonData, itemType, sourceId, compendium) {
	...
	try {
		...
		console.log(error);
		const message =
			"Could not create item from file data, name: " +
			name +
			", type " +
			Utils.getLocalisedItemType(itemType);
 ...
 */
async function writeNewCompendiumItems(
	relevantData,
	compendium,
	itemType,
	fileId,
...
export async function deleteFileRecord(fileId, dialog) {
	for (const compendium of game.packs) {
		dialog.updateLoadingMessage(
			`${game.i18n.localize("MANAGER.removing")} ${compendium.metadata.label}`
		);
...
async function removeItemsFromFileSource(compendium, fileId) {
	...
	const existingItems = await index.filter((item) =>
		isItemFromFileSource(item, fileId)
	);
	const toDelete = [];
	existingItems.forEach((item) => {
		toDelete.push(item._id);
	});
	await compendium.documentClass.deleteDocuments(toDelete, {
